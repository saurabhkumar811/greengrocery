...
const Cart = () => {
     ...
     try {
        if(!selectedAddresses){
          ...
          if(paymentOption === 'COD'){
            const items = Object.entries(cartItems).map(([product, quantity]) => ({
              product,
              quantity
            }));
         ...
         const {data} = await axios.post("/api/order/cod",{
           userId: user._id,
           items,
           address: selectedAddresses._id, // Also use _id, not id, for MongoDB documents
         })
  ...
  useEffect(()=>{
   if(products.length > 0  && Object.keys(cartItems).length){
     getCart()
   }
  },[products, cartItems])
...
<div className="flex flex-col md:flex-row md:justify-between mt-6">
  ...
  <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70 md:ml-8">
